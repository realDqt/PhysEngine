...
void Voxelize(vec3r* vertices, int numVertices, const int* indices, int numTriangleIndices, uint32_t width, uint32_t height, uint32_t depth, uint32_t* volume, vec3r minExtents, vec3r maxExtents)
{
	...
	AABBTree tree(vertices, numVertices, (uint32_t*)indices, numTriangleIndices/3); 

	// parity count method, single pass
	const vec3r extents(maxExtents-minExtents);
	const vec3r delta = make_vec3r(extents.x/width, extents.y/height, extents.z/depth);
	const vec3r offset = make_vec3r(0.5f*delta.x, 0.5f*delta.y, 0.5f*delta.z);
	...
	for (uint32_t x=0; x < width; ++x)
	{
		for (uint32_t y=0; y < height; ++y)
		{
			...
			vec3r rayDir = make_vec3r(0.0f, 0.0f, 1.0f);
			vec3r rayStart = minExtents + make_vec3r(x*delta.x + offset.x, y*delta.y + offset.y, 0.0f);

...
AABBTree::AABBTree(vec3r* vertices, uint32_t numVerts, uint32_t* indices, uint32_t numFaces) 
    : m_vertices(vertices)
    , m_numVerts(numVerts)
    , m_indices(indices)
    , m_numFaces(numFaces)
...
namespace
{
	...
	struct FaceSorter
	{
		FaceSorter(const vec3r* positions, const uint32_t* indices, uint32_t n, uint32_t axis) 
		...
		{
			const vec3r& a = m_vertices[m_indices[face*3+0]];
			const vec3r& b = m_vertices[m_indices[face*3+1]];
			const vec3r& c = m_vertices[m_indices[face*3+2]];
	...
	inline uint32_t LongestAxis(const vec3r& v)
	{    
		if (v.x > v.y && v.x > v.z)
			return 0;
		else
...
void AABBTree::CalculateFaceBounds(uint32_t* faces, uint32_t numFaces, vec3r& outMinExtents, vec3r& outMaxExtents)
{
    vec3r minExtents = make_vec3r(FLT_MAX);
    vec3r maxExtents = make_vec3r(-FLT_MAX);

    // calculate face bounds
    for (uint32_t i=0; i < numFaces; ++i)
    {
        vec3r a = m_vertices[m_indices[faces[i]*3+0]];
        vec3r b = m_vertices[m_indices[faces[i]*3+1]];
        vec3r c = m_vertices[m_indices[faces[i]*3+2]];
...
/*
bool AABBTree::TraceRay(const vec3r& start, const Vector3& dir, float& outT, float& outU, float& outV, float& outW, float& outSign, uint32_t& outIndex) const
{
#if _WIN32
    // reset stats
    s_traceDepth = 0;
...
filth:
        ...
        else
        {
            for (uint32_t i=0; i < node->m_numFaces; ++i)
            {
                ...
                const vec3r& a = m_vertices[m_indices[indexStart+0]];
                const vec3r& b = m_vertices[m_indices[indexStart+1]];
                const vec3r& c = m_vertices[m_indices[indexStart+2]];
#if TRACE_STATS
...
bool AABBTree::TraceRay(vec3r& start, vec3r& dir, float& outT, float& u, float& v, float& w, float& faceSign, uint32_t& faceIndex)
{   
    //s_traceDepth = 0;

    vec3r rcp_dir = make_vec3r(1.0f/dir.x, 1.0f/dir.y, 1.0f/dir.z);
...
void AABBTree::TraceRecursive(uint32_t nodeIndex, vec3r& start, vec3r& dir, float& outT, float& outU, float& outV, float& outW, float& faceSign, uint32_t& faceIndex)
{
    ...
    if (node.m_faces == NULL)
    {
        ...
        // find closest node
        Node& leftChild = m_nodes[node.m_children+0];
        Node& rightChild = m_nodes[node.m_children+1];

        float dist[2] = {FLT_MAX, FLT_MAX};

        IntersectRayAABB(start, dir, leftChild.m_minExtents, leftChild.m_maxExtents, dist[0]);
        IntersectRayAABB(start, dir, rightChild.m_minExtents, rightChild.m_maxExtents, dist[1]);
    ...
    else
    {
        ...
        for (uint32_t i=0; i < node.m_numFaces; ++i)
        {
            ...
            vec3r& a = m_vertices[m_indices[indexStart+0]];
            vec3r& b = m_vertices[m_indices[indexStart+1]];
            vec3r& c = m_vertices[m_indices[indexStart+2]];
#if TRACE_STATS
            extern uint32_t g_trisChecked;
            ++g_trisChecked;
#endif
...
/*
bool AABBTree::TraceRay(const vec3r& start, const Vector3& dir, float& outT, Vector3* outNormal) const
{   
    outT = FLT_MAX;
...
void AABBTree::TraceRecursive(uint32_t n, const vec3r& start, const Vector3& dir, float& outT, Vector3* outNormal) const
{
...
*/
bool AABBTree::TraceRaySlow(vec3r& start, vec3r& dir, float& outT, float& outU, float& outV, float& outW, float& faceSign, uint32_t& faceIndex)
{    
    ...
    vec3r minNormal = make_vec3r(0.0f, 1.0f, 0.0f);

    vec3r n = make_vec3r(0.0f, 1.0f, 0.0f);
    float t, u, v, w, s;
    bool hit = false;
	uint32_t minIndex = 0;
    ...
    for (uint32_t i=0; i < numFaces; ++i)
    {
        vec3r& a = m_vertices[m_indices[i*3+0]];
        vec3r& b = m_vertices[m_indices[i*3+1]];
        vec3r& c = m_vertices[m_indices[i*3+2]];
...
void AABBTree::DebugDrawRecursive(uint32_t nodeIndex, uint32_t depth)
{
    ...
    n.m_center = vec3r(minExtents+maxExtents)/2;
...
vec3r Min(vec3r a, vec3r b) {
    return make_vec3r(std::min(a.x, b.x), std::min(a.y, b.y), std::min(a.z, b.z));
}
vec3r Max(vec3r a, vec3r b) {
    return make_vec3r(std::max(a.x, b.x), std::max(a.y, b.y), std::max(a.z, b.z));
}

bool IntersectRayAABB(vec3r start, vec3r dir, vec3r min, const vec3r max, float& t)
{
